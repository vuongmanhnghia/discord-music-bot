...
class PlaylistManager:
    ...
    def get_playlists(self) -> List[PlaylistInfo]:
        ...
        playlists = []
        for file_path in config.spotdl_path.glob("*.spotdl"):
            name = file_path.stem
            ...
            song_count = 0
            if music_dir.exists():
                song_count = len([f for f in music_dir.iterdir() 
    ...
    def get_playlist(self, name: str) -> Optional[PlaylistInfo]:
        ...
        file_path = config.spotdl_path / f"{name}.spotdl"
    ...
    def find_song(self, song_name: str, playlist_name: Optional[str] = None) -> Optional[str]:
        ...
        # Try partial match
        for file_path in music_dir.iterdir():
            if (file_path.suffix.lower() in self.AUDIO_EXTENSIONS and 
                song_name.lower() in file_path.stem.lower()):
    ...
    def ensure_sync_format(self, playlist_name: str) -> bool:
        ...
        file_path = config.spotdl_path / f"{playlist_name}.spotdl"
        if not file_path.exists():
            return False
