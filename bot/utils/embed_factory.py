...
class EmbedFactory:
    ...
    def create_music_embed(
        title: str, description: str = "", song=None, **kwargs
    ) -> discord.Embed:
    ...
    def create_queue_embed(
        current_song=None, queue_list=None, page: int = 1, **kwargs
    ) -> discord.Embed:
        """Create a queue display embed"""
        embed = discord.Embed(
            title=f"{EMOJIS['queue']} Hàng đợi nhạc", color=COLORS["info"]
        )
        ...
        if queue_list:
            ...
            embed.add_field(name="📄 Danh sách", value="Hàng đợi trống", inline=False)

        return EmbedFactory._add_common_fields(embed, **kwargs)

    @staticmethod
    def create_playlist_embed(
        playlist_name: str, songs: list, **kwargs
    ) -> discord.Embed:
    ...
    def _add_common_fields(
        embed: discord.Embed, footer: Optional[str] = None, **kwargs
    ) -> discord.Embed:
