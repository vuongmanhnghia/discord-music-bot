...
class MusicEmbedBuilder:
    ...
    @staticmethod
    def create_now_playing_embed(song, guild_id: int, audio_player=None) -> discord.Embed:
        """Create detailed now playing embed"""
        embed = EmbedFactory.create_music_embed("Đang phát", song.display_name, song=song)
        ...
        if audio_player:
            ...
            embed.add_field(name="Âm lượng", value=f"{volume_emoji} {volume}%", inline=True)
        
        return embed
    
    @staticmethod
    def create_play_success_embed(song, queue_position: Optional[int] = None) -> discord.Embed:
        """Create embed for successful play request"""
        embed = EmbedFactory.create_success_embed("Đã thêm vào hàng đợi", song.display_name)
        ...
        if queue_position is not None:
            embed.add_field(name="Vị trí queue", value=f"#{queue_position}", inline=True)
    ...
    def create_playlist_progress_embed(
        ...
        embed.add_field(name="⏳ Tiến độ", value=f"{current_index}/{total_count}", inline=True)
...
class SystemEmbedBuilder:
    ...
    def create_resource_stats_embed(stats: Dict[str, Any]) -> discord.Embed:
        ...
        embed = EmbedFactory.create_info_embed(
            "Bot Resource Statistics",
            "Thống kê sử dụng tài nguyên và hiệu suất"
        )
        ...
        status_emoji = "🟢" if active_connections < 8 else "🟡"
    ...
    def create_cache_performance_embed(cache_stats: Dict[str, Any]) -> discord.Embed:
        ...
        embed = EmbedFactory.create_info_embed(
            ...
            "Thống kê hiệu suất cache thông minh"
        )
        
        hit_rate = cache_stats.get('hit_rate', 0)
        status_emoji = "🟢" if hit_rate > 50 else "🟡"
...
class HelpEmbedBuilder:
    ...
    def create_main_help_embed(bot_name: str) -> discord.Embed:
        ...
        embed.add_field(name="▶️ Phát nhạc", value="\n".join(playback_cmds), inline=False)
    ...
    def create_url_handling_info_embed() -> discord.Embed:
        ...
        embed = EmbedFactory.create_info_embed(
            ...
            "Cách bot xử lý các loại URL YouTube khác nhau"
        )
