...
class YouTubePlaylistHandler:
    ...
    def is_playlist_url(url: str) -> bool:
        if not url or not isinstance(url, str):
            return False
        playlist_patterns = [r"youtube\.com/playlist\?list=", r"music\.youtube\.com/playlist\?list="]
    ...
    def is_single_video_with_playlist(url: str) -> bool:
        if not url or not isinstance(url, str):
            return False
        single_video_patterns = [r"youtube\.com/watch\?.*v=.*&.*list=", r"youtube\.com/watch\?.*list=.*&.*v=", r"youtu\.be/.*\?.*list="]
        return any(re.search(pattern, url, re.IGNORECASE) for pattern in single_video_patterns)
    
    @staticmethod
    def extract_playlist_id(url: str) -> Optional[str]:
        try:
            parsed_url = urlparse(url)
            if parsed_url.netloc in ["www.youtube.com", "youtube.com", "music.youtube.com", "youtu.be"]:
    ...
    async def extract_playlist_videos(url: str) -> Tuple[bool, List[str], str]:
        try:
            ...
            ydl_opts = {"quiet": True, "no_warnings": True, "extract_flat": True, "playlist_items": "1-100"}
            video_urls = []
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                try:
                    ...
                    if not entries:
                        return False, [], f"Playlist '{playlist_title}' is empty or private"
                    ...
                    if not video_urls:
                        return False, [], f"No accessible videos found in playlist '{playlist_title}'"
    ...
    @staticmethod
    def format_playlist_message(video_count: int, playlist_title: str, processing_type: str = "added") -> str:
        if video_count == 0:
            return f"❌ No videos found in playlist '{playlist_title}'"
        elif video_count == 1:
            return f"✅ {processing_type.title()} 1 video from playlist '{playlist_title}'"
        else:
...
class YouTubeErrorHandler:
    ...
    def is_403_error(self, error_msg: str) -> bool:
        error_indicators = ["403 Forbidden", "Server returned 403", "HTTP error 403", "access denied", "Access denied"]
    ...
    async def get_retry_delay(self, attempt: int) -> int:
        return self._retry_delays[attempt] if attempt < len(self._retry_delays) else self._retry_delays[-1]
    ...
    def clean_old_403_records(self, max_age_hours: int = 24):
        ...
        urls_to_remove = [url for url, timestamp in self._last_403_time.items() if timestamp < cutoff_time]
