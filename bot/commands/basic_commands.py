    ...
    def setup_commands(self):
            ...
            try:
                ...
                if interaction.guild.voice_client:
                    if interaction.guild.voice_client.channel == user_voice_channel:
                        await interaction.response.send_message(
                            SUCCESS_MESSAGES['connected'].format(user_voice_channel.name), ephemeral=True
                        )
                        return
                    else:
                        # Move to new channel
                        ...
                        await interaction.response.send_message(
                            SUCCESS_MESSAGES['moved_channel'].format(user_voice_channel.name)
                        )
                        return
                ...
                try:
                    ...
                    await audio_service.initialize_guild(interaction.guild.id, voice_client)
                    
                    await interaction.response.send_message(
                        SUCCESS_MESSAGES['connected'].format(user_voice_channel.name)
                    )
                    
                    logger.info(f"Connected to voice channel: {user_voice_channel.name} in {interaction.guild.name}")
                    
                except discord.ClientException as e:
                    error_msg = f"Không thể kết nối voice channel: {str(e)}"
        ...
        async def leave_command(interaction: discord.Interaction):
            ...
            try:
                ...
                await interaction.response.send_message(f"👋 Đã rời khỏi **{channel_name}**!")
                
                logger.info(f"Left voice channel: {channel_name} in {interaction.guild.name}")
