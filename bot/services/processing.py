...
class SpotifyService(SongProcessor):
    ...
    async def process(self, song: Song) -> bool:
        ...
        try:
            ...
            logger.info(f"Successfully processed Spotify track: {metadata.display_name}")
            return True
...
class YouTubeService(SongProcessor):
    ...
    async def search_for_spotify_song(self, song: Song) -> bool:
        ...
        logger.info(f"Searching YouTube for Spotify track: {song.metadata.display_name}")
        ...
        try:
            ...
            logger.info(f"Found YouTube stream for Spotify track: {song.metadata.display_name}")
            return True
    ...
    async def _extract_youtube_data(self, query: str) -> Optional[tuple[SongMetadata, str]]:
        ...
        try:
            ...
            stdout, stderr = await asyncio.wait_for(metadata_process.communicate(), timeout=30)
    ...
    async def process_song(self, song: Song) -> bool:
        ...
        try:
            ...
            if success and song.source_type == SourceType.SPOTIFY and not song.stream_url:
                ...
                youtube_success = await self.youtube_service.search_for_spotify_song(song)
