...
class StreamRefreshService:
    ...
    async def should_refresh_url(self, song: Song) -> bool:
        ...
        if hasattr(song, 'stream_url_timestamp'):
            ...
            if age > STREAM_URL_MAX_AGE:
                logger.info(f"🔄 Stream URL for '{song.display_name}' is {age/3600:.1f} hours old, needs refresh")
                return True
    ...
    async def refresh_stream_url(self, song: Song) -> bool:
        ...
        try:
            ...
            from ..services.processing import YouTubeProcessor
            ...
            if new_stream_url:
                ...
                logger.info(f"✅ Stream URL refreshed for '{song.display_name}' (#{self.refresh_count})")
    ...
    def get_refresh_stats(self) -> dict:
        ...
        return {
            ...
            "time_since_last_refresh": current_time - self.last_refresh_time if self.last_refresh_time > 0 else 0
        }
