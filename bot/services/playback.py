...
class PlaybackService:
    ...
    async def play_request(self, 
                          ...
                          guild_id: int, 
                          requested_by: str,
                          auto_play: bool = True) -> tuple[bool, str, Optional[Song]]:
        """
        ...
        """
        logger.info(f"Processing play request: '{user_input}' from {requested_by} in guild {guild_id}")
        ...
        try:
            ...
            logger.info(f"Analyzed input: {song.source_type.value} - {song.original_input}")
            
            # Step 2: Add to queue immediately (in pending state)
            ...
            # Step 3: Process song asynchronously
            await self._start_song_processing(song)
            ...
            return (
                True, 
                f"Added **{song.display_name}** to queue (position {position})",
                song
            )
    ...
    async def _process_song_with_callbacks(self, song: Song):
        ...
        try:
            ...
            if success:
                ...
                logger.warning(f"Failed to process: {song.display_name} - {song.error_message}")
    ...
    async def _try_start_playback(self, guild_id: int):
        ...
        try:
            ...
            if not audio_service.is_connected(guild_id):
                logger.debug(f"Not connected to voice in guild {guild_id}, cannot start playback")
                return
            ...
            # Start playback
            logger.info(f"Starting playback for guild {guild_id}: {current_song.display_name}")
